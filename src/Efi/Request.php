        ...
        if (isset($certificate)) {
            if (file_exists($certPath = realpath($certificate))) {
                if (!$fileContents = file_get_contents($certPath)) {
                    throw new EfiPayException(['nome' => 'forbidden', 'mensagem' => 'Unable to read the cert file'], 403);
                }

                if (strtolower(substr($certPath, -3)) === 'p12') {
                    if (!openssl_pkcs12_read($fileContents, $certData, $password = '')) {
                        throw new EfiPayException(['nome' => 'forbidden', 'mensagem' => 'Unable to read the cert file p12'], 403);
                    }
                ...
                if (!$publicKey = openssl_x509_parse($fileContents)) {
                    throw new EfiPayException(['nome' => 'forbidden', 'mensagem' => 'Certificate invalid'], 403);
                }
                ...
                if ($validTo <= $today) {
                    throw new EfiPayException(['nome' => 'forbidden', 'mensagem' => 'Authentication certificate expired on ' . $validTo], 403);
                }

                return $certPath;
            } else {
                throw new EfiPayException(['nome' => 'forbidden', 'mensagem' => 'Certificate not found'], 403);
            }
        }
    }
